...
(async()=>{
 ...
 try{
  ...
  const result=await page.evaluate(async()=>{state.modelCatalog.gemini=['gemini-3.8-flash','gemini-3.5-flash'];state.geminiRuntime={cursor:0,usage:{},lastModel:''};state.geminiModelByQuestion={};return runGeminiWithFallback({mode:'gemini',model:'auto',apiKey:'test-only'},()=>({contents:[{parts:[{text:'test'}]}]}),'평가',1);});
